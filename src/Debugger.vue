...
<template lang="pug">
  .vue-debugger-container(@click.prevent.stop="")
    transition(name="debugger-highlight")
      .vue-debugger.highlighter(
        ...
        h4 {{ targeted.$options._componentTag }}

    .vue-debugger.pane(:class="{ opened: open }")
      .drag-handle(@mousedown.stop.prevent="dragging = true")
      .vue-debugger.target(:class="{ active: targeting }", @click.prevent="toggleTargeting") ◎
      .vue-debugger.toggle(@dblclick="clearStore", @click.prevent="toggle") 🔮
      ...
      .vue-debugger.main-pane(v-if="keepAlive || open")
        obj-tree(v-for="value, key in dataSource", :name="key", :value="value", :key="key")
